...
# Default: verify certificates normally (safe). Some corporate networks run
# HTTPS-inspecting proxies/VPNs/antivirus that swap in their own certificate
# for every TLS connection - `requests` (correctly) rejects that as an
# untrusted cert, which surfaces as check_connectivity()'s SSLError message.
#
# The *proper* fix needs no code change at all: `requests` already honors
# the standard `REQUESTS_CA_BUNDLE` (or `SSL_CERT_FILE`) environment
# variable. Ask your IT/security team for the org's root CA as a .pem file,
# then before launching the app:
#   Windows (PowerShell):  $env:REQUESTS_CA_BUNDLE = "C:\path\to\corp-ca.pem"
#   macOS/Linux:            export REQUESTS_CA_BUNDLE=/path/to/corp-ca.pem
# `requests` will then trust that proxy's certificate and everything works
# with full verification intact.
#
# When that isn't available (e.g. you don't have IT access), `set_ssl_verify(False)`
# below is a fallback that disables certificate verification for calls to
# this one read-only, non-sensitive public API only - never use it more
# broadly. It can also be turned on via the PINCODE_API_SSL_VERIFY=0 env var.
_ssl_verify = os.environ.get("PINCODE_API_SSL_VERIFY", "1") != "0"
_ssl_verify_lock = threading.Lock()


def set_ssl_verify(enabled: bool):
    """Toggle certificate verification for the pincode API only.

    Disabling this means a network attacker positioned between this
    machine and GitHub Pages (e.g. on public wifi) could tamper with
    pincode lookup responses undetected - low-stakes here since this API
    only returns public geographic data, but still off by default.
    """
    global _ssl_verify
    with _ssl_verify_lock:
        _ssl_verify = bool(enabled)
        if not _ssl_verify:
...
def check_connectivity(timeout: int = 6) -> dict:
    ...
    start = time.time()
    try:
        resp = _session.get(f"{BASE_URL}/pincodes/{test_pin}.json", timeout=(4, timeout), verify=get_ssl_verify())
        ...
        return {"ok": False,
                 "message": ("TLS/SSL error - a proxy, VPN, or antivirus on this network is intercepting HTTPS "
                              "traffic with its own certificate. Proper fix: get your org's root CA .pem from IT "
                              "and set the REQUESTS_CA_BUNDLE environment variable to its path before launching "
                              "the app. Quick workaround: enable 'Ignore SSL errors (corporate proxy)' in the "
                              f"sidebar. ({e})"),
                 "elapsed": time.time() - start}
...
def _fetch_one(pincode: str, timeout: int):
    ...
    try:
        resp = _session.get(f"{BASE_URL}/pincodes/{pincode}.json", timeout=(4, timeout), verify=get_ssl_verify())
