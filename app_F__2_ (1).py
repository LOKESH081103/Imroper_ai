...
def create_report_image(today_m, yest_m, extra_t, extra_y, report_date, view_key):
    ...
    font_title    = _load_font(168)
    font_subtitle = _load_font(74)
    font_section  = _load_font(84)
    font_header   = _load_font(60)
    font_body     = _load_font(55)
    font_small    = _load_font(48)

    y = 0
    # Header
    draw.rectangle([0, 0, width, 400], fill=COLOR_HEADER_BG)
    draw.text((95, 60), "DAILY COLLECTIONS UPDATE", fill=COLOR_HEADER_TEXT, font=font_title)
    draw.text((95, 259), f"{label}  |  {report_date}", fill=COLOR_HEADER_TEXT, font=font_subtitle)
    y = 510
    ...
    def draw_section_header(title):
        nonlocal y
        draw.rectangle([60, y, width - 60, y + 145], fill=COLOR_SECTION_BG)
        draw.text((95, y + 30), title, fill=COLOR_SECTION_TEXT, font=font_section)
        y += 170

    def draw_table_header(headers, col_x):
        nonlocal y
        draw.rectangle([60, y, width - 60, y + 145], fill=COLOR_GRID_HEADER_BG)
        for i, h in enumerate(headers):
            draw.text((col_x[i] + 18, y + 42), h, fill=COLOR_GRID_HEADER_T, font=font_header)
        draw.line([60, y + 145, width - 60, y + 145], fill=COLOR_GRID_BORDER, width=3)
        y += 150
