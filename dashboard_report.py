...
<body>
  ...
  <div class="kpis">
    ...
    {gt_cards}
    <div class="card"><p class="label">Flagged by rule engine (Layers 1+3)</p><p class="value">{rule_flagged_fmt} <span style="font-size:14px;color:var(--muted)">({rule_flagged_pct}%)</span></p></div>
    {accuracy_card}
  </div>
...
def render_html(data: dict, source_name: str, title: str) -> str:
    ...
    banner = ('<div class="banner"><span>&#9888;</span><span>'
              ...
              'call per unique pincode. Run the Streamlit app for full 4-layer numbers.</span></div>')
...
<table>
  ...
  <tr><td>Actually proper</td><td>{c["fp"]:,}</td><td>{c["tn"]:,}</td></tr>
        ...
        mcmp = data["model_comparison"]
        if mcmp and "skipped_reason" not in mcmp:
            algo_label = {"xgb": "XGBoost", "logreg": "Logistic Regression", "nb": "Naive Bayes"}
            algs = list(mcmp.keys())
            colors = ["#2a78d6", "#eda100", "#008300"]
            datasets_js = ",\n      ".join(
                f'{{ label: "{algo_label.get(a,a)}", data: {json.dumps([mcmp[a]["accuracy"], mcmp[a]["precision"], mcmp[a]["recall"], mcmp[a]["f1"]])}, backgroundColor: "{colors[i % 3]}", borderRadius: 4 }}'
                for i, a in enumerate(algs)
            )
            legend_html = "".join(
                f'<span><span class="dot" style="background:{colors[i % 3]}"></span>{algo_label.get(a,a)}</span>'
                for i, a in enumerate(algs)
            )
            model_block = f"""
...
<p class="note">xgboost not installed in this environment is automatically skipped above if absent - install it (pip install xgboost) and re-run to include it.</p>
"""
            model_chart_js = f"""
new Chart(document.getElementById('modelChart'), {{
  ...
  options: {{ ...common, plugins: {{ legend: {{ display: false }} }},
    scales: {{ y: {{ min: 0, max: 100, grid: {{ color: grid }}, ticks: {{ color: muted }} }}, x: {{ grid: {{ display: false }}, ticks: {{ color: muted }} }} }} }}
            ...
            reason = (mcmp or {}).get("skipped_reason", "not enough labeled data")
            model_block = f'<h2>Layer 4 model comparison</h2><p class="note">Skipped: {reason}</p>'
            model_chart_js = ""
    else:
        model_chart_js = ""
    ...
    return HTML_TEMPLATE.format(
        ...
        model_chart_js=model_chart_js if data["has_ground_truth"] else "",
    )
